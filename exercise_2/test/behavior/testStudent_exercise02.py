...
class TestObsToState(unittest.TestCase):
    ...
    def test_mapping(self):
        try:
            self.assertTrue(
                ...
                "Position and velocity are incorrectly mapped.",
            )
        except AssertionError as e:
            self.assertFalse(
                np.allclose(self.state[[0, 1, 2]], self.state_expected[[0, 2, 4]]),
                "Incorrect mapping: Position and velocity are not assigned to the correct indices.\n"
                "Expected order is [x, x_dot, y, y_dot, z, z_dot].",
            )
            raise e
    ...
    def test_euler_angle(self):
        ...
        try:
            ...
            self.assertFalse(
                np.allclose(self.state[[6, 7, 8]], self.state_expected[[8, 7, 6]]),
