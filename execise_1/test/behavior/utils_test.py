...
def restore_attributes_from_dict(
    obj: Any, attributes: Optional[Dict[str, Any]]
) -> None:
...
def compare_dicts(
    obj: unittest.TestCase, dict1, dict2, rtol=1e-4, atol=1e-5, key_list=[]
):
    ...
    for key in dict1:
        ...
        if isinstance(dict1[key], dict):
            compare_dicts(
                obj, dict1[key], dict2[key], rtol=rtol, atol=atol, key_list=key_list
            )
            ...
            obj.assertEqual(
                len(dict1[key]),
                len(dict2[key]),
                f"List lengths differ for key '{key_list}'",
            )
            for item1, item2 in zip(dict1[key], dict2[key]):
                compare_dicts(
                    obj, item1, item2, rtol=rtol, atol=atol, key_list=key_list
                )
            ...
            obj.assertAlmostEqual(
                ...
                dict2[key],
                delta=atol,
                msg=f"Values differ for key '{key_list}'",
            )
            ...
            obj.assertEqual(
                dict1[key], dict2[key], f"Values differ for key '{key_list}'"
            )


def gen_error_msg_array(
    name, computed, reference, rtol, atol, max_mismatches=2, only_name=True
):
